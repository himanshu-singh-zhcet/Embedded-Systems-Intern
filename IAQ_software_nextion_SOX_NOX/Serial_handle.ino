void serialHandle() {
  if (Serial.available()) {
    ...
    if (s.startsWith("debug")) {                             //// debug mode
      ...
      Serial.print(F("K Fac co    : ")); Serial.println(noxcalib);
      Serial.print(F("K Fac o3    : ")); Serial.println(soxcalib);
    }
    ...
    //    }
      ...
      Serial.println("co Fac: " + String(soxcalib));
    }
    else if (s.startsWith("o3")) {
      s.remove(0, s.indexOf(':') + 1);
      noxcalib = s.toFloat();
      EEPROM.put(E_noxcalib, noxcalib);
      Serial.println("o3 Fac: " + String(noxcalib));
    }
    else if (s.indexOf("##$$&&") > -1) {
