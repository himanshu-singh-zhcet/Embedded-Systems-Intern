...
void getSensorValue() {
  ...
  para_val[SOX] = ((v2 - 0.6) * 20 / 2.4);     // 20 / 2.4 * (v1 - soxZeroPoint);
  para_val[NOX] = abs((v1 - 2.0) * 20 / 2.0);    //20 / 2.0 * (v2 - noxZeroPoint);

  if (debug) {
    Serial.println("Volts1: " + String(v1));
    Serial.println("Volts2: " + String(v2));

    Serial.println("raw sox: " + String(para_val[SOX]));
    Serial.println("raw nox: " + String(para_val[NOX]));
  }

  para_val[SOX] *= soxcalib;
  para_val[NOX] *= noxcalib;
