void deviceConfigure() {
  ...
  PumpTime = EEPROM.read(E_SAMPLEINTERVAL);

  EEPROM.get(E_DEVICE_INFO, device);
  EEPROM.get(E_noxcalib, noxcalib);
  EEPROM.get(E_soxcalib, soxcalib);
  EEPROM.get(E_pm25calib, pm25calib);
  EEPROM.get(E_pm10calib, pm10calib);
  EEPROM.get(E_APN, apn);
  ...
  Serial.println("PM ANALYZER | EES | 2022");
  printDash(50);
}
