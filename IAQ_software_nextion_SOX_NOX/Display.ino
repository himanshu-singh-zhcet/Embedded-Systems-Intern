  ...
  if (displaySerial.available()) {
    ...
    while (response1.indexOf("$") > -1) {
      ...
      if (response == F("p_menu")) {
        ...
        //        for nextion 3.5
        //        sendThis = "t2.txt=\"" + String(soxcalib) + "\"";
        //        writeString(sendThis);
        //        sendThis = "t3.txt=\"" + String(noxcalib) + "\"";
        //        writeString(sendThis);

        //        for nextion 7
        sendThis = "pHCalib.t0.txt=\"" + String(soxcalib) + "\"";
        writeString(sendThis);
        sendThis = "tssCalib.t0.txt=\"" + String(noxcalib) + "\"";
        writeString(sendThis);
        ...
        Serial.println(p);
        if (p) {
          noxcalib = response.toFloat();
          EEPROM.put(E_noxcalib, noxcalib);
          //        EEPROM.get(E_noxcalib, noxcalib);
