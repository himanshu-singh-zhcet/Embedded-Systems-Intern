...
bool saveToSD() {
  ...
  if (file) {
    if (write_header) {
      String headder = F("Date,Time");  //PM2.5(ug/m3),PM10(ug/m3),SOX(PPM),CO2(PPM)");
      for (int i = 0; i < no_para; i++) {
        headder = headder + "," + para[i] + " (" + para_unit[i] + ")";
      }
    ...
    for (int i = 0; i < no_para; i++) {
      String savePara = "," + para_val_str[i];
      file.print(savePara);
    }
    file.println();
    ret = true;
