...
// EEPROM Declare
#define E_CONFIG    0
#define E_soxcalib  10
#define E_noxcalib  20
...
////////////////parameters/////////////////
const uint8_t no_para = 4;
enum para_enum {PM2_5, PM10, TEMP, HUM};
String para[no_para] = {"PM2_5", "PM10", "TEMP", "HUM"};
String para_unit[no_para] = {"ug/m^3", "ug/m^3", "*C", "%"};
String para_val_str[no_para] = {"0", "0", "0", "0"};
...
float noxcalib , soxcalib, pm25calib , pm10calib;
ISR (WDT_vect) {
  rst_counter++;
  //  pumpInterval++;
...
void loop() {
  ...
  if (millis() - pm_lt > 5000L) {
    ...
    au16data[0] = (uint16_t)(para_val[PM2_5]);
    au16data[1] = (uint16_t)(para_val[PM10]);
    au16data[2] = (uint16_t)(para_val[TEMP] * 100.0);
    au16data[3] = (uint16_t)(para_val[HUM] * 100.0);
