...
void sendDisplayVal() {
  ...
  uint8_t a[paraPerDisp] = {paraPerDisp * k, paraPerDisp * k + 1, paraPerDisp * k + 2, paraPerDisp * k + 3};

  for (int i = 0; i < paraPerDisp; i++) {
    if (a[i] < no_para) {
      sendThis = page + ".para" + String(i) + ".txt=\"" + para[a[i]] + "\"";
      writeString(sendThis);
      sendThis = page + ".value" + String(i) + ".txt=\"" + para_val_str[a[i]] + "\"";
      writeString(sendThis);
      sendThis = page + ".unit" + String(i) + ".txt=\"" + para_unit[a[i]] + "\"";
      writeString(sendThis);
    }
    else {
      sendThis = page + ".para" + String(i) + ".txt=\"-\"";
      writeString(sendThis);
      sendThis = page + ".value" + String(i) + ".txt=\"-\"";
      writeString(sendThis);
      sendThis = page + ".unit" + String(i) + ".txt=\"-\"";
      writeString(sendThis);
    }
  }
  k++;
